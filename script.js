...
const handle = async (e) => {
  // for making the enter button to work
    if (e.keyCode == 13) {
        const word = e.target.value;
        // to make a call request to the api
        const result = await fetch(url + word);
// to make the result as view to UI
        resultDiv.style.display = "block";
        const data = await result.json();
        // print the searched word
    alert(data[0].word);
// if the api has the input,the result is ok
        if (result.ok) {
            ...
            let synonymsData = "";
            // to print the multiple words in synonyms
        ...
        // if the input is not defined or not found in dictionary.
         else {
